...
public class Com_verificacao_de_indice {
	...
	public static void mult(int[][] matrizA, int[][]matrizB, ArrayList<int[][]> Multmatrizes,ArrayList<int[][]> matrizes,int cont) {
		...
		for (int i = 0; i < matrizB.length; i++) 
		{
			for (int j = 0; j < matrizB.length; j++) 
			{
				...
				{
					if(verificar(i, j, k, cont, matrizes)) {
	...
	//Faz a verificação de cada item
	public static boolean verificar(int i,int j,int k, int cont,ArrayList<int[][]> matrizes ) {
		boolean result = true;
		int test;
		try {
			test = matrizes.get(cont)[i][k];
			test = matrizes.get(cont+1)[k][j];
